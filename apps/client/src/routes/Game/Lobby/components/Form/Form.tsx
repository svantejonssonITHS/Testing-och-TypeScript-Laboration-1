...
export default function Form({ options }: FormProps): JSX.Element {
	const [regions]: [Item[], React.Dispatch<React.SetStateAction<Item[]>>] = useState(options.regions);
	const [region, setRegion]: [Item | undefined, React.Dispatch<React.SetStateAction<Item | undefined>>] = useState();
	const [categories]: [Item[], React.Dispatch<React.SetStateAction<Item[]>>] = useState(options.categories);
	const [category, setCategory]: [Item | undefined, React.Dispatch<React.SetStateAction<Item | undefined>>] =
		useState();
	const [tags]: [Item[], React.Dispatch<React.SetStateAction<Item[]>>] = useState(options.tags);
	const [tag, setTag]: [Item | undefined, React.Dispatch<React.SetStateAction<Item | undefined>>] = useState();
	const [difficulties]: [Item[], React.Dispatch<React.SetStateAction<Item[]>>] = useState(options.difficulties);
